    ...
    # Create as many trackers as there are video sources
    	...
    	for i in range(nr_sources):
        	deepsort_list.append(
            		DeepSort(deep_sort_model, device, max_dist=cfg.DEEPSORT.MAX_DIST,
        ...
        # Process detections
			...
			outputs[i] = deepsort_list[i].update(xywhs.cpu(), confs.cpu(), clss.cpu(), im0)
				...
				if poi_id is None:
		    		...
		    		print("pose {:}, id {:}".format(poi_id, id_pred))
                    		if not up:
                    			continue
		    		if poi_id is None:
		    			poi_id = ouputs[i,id_pred,4]
		    
                    		if poi_id is not None and poi_id in outputs[i,:,4]:
					print("track")
