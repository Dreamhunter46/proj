...
class Tracker:
    ...
    def set_ds_idx(self):
        self.detection_idx = []
        self.tracking_unmatched_idx = []
        
    def add_ds_idx(self, det_idx, track_idx):
        self.detection_idx.append((det_idx, track_idx))
        
    def get_ds_idx(self):
        return self.detection_idx
    
    def get_ut_ds_idx(self):
        return self.tracking_unmatched_idx
    ...
    def update(self, detections, classes, confidences):
        ...
        # Update track set.
        self.set_ds_idx()
        self.detection_idx = matches
        self.tracking_unmatched_idx = unmatched_tracks
        for track_idx, detection_idx in matches:
            #print('Match: track idx {} updates with yolo detection {}'.format(track_idx, detection_idx))
            self.tracks[track_idx].update(
                self.kf, detections[detection_idx], classes[detection_idx], confidences[detection_idx])
        for track_idx in unmatched_tracks:
            #print('Unmatched: track idx {}'.format(track_idx))
            self.tracks[track_idx].mark_missed()
        for detection_idx in unmatched_detections:
            #print('Unmatched detection NEW TRACK with track idx {}'.format(self._next_id))
    ...
    def _match(self, detections):
        ...
        #print('self.counter_init: {}'.format(self.counter_init))
        ...
        matches_a, unmatched_tracks_a, unmatched_detections = linear_assignment.matching_cascade(
            ...
            self.metric.matching_threshold,  #linear_assignment.INFTY_COST - 1,  # no need for self.metric.matching_threshold here,
            self.max_age,
            self.tracks,
            detections,
            confirmed_tracks,
        )
        #print('matches_a: {}'.format(matches_a))
        
        if self.counter_init >= 0: #< 5:
            #print('IOU')
            ...
            #print('matches_b: {}'.format(matches_b))
